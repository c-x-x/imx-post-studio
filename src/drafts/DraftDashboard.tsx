...
export function DraftDashboard({ onOpen, disabled = false }: DraftDashboardProps) {
  ...
  return <section ref={dashboardRef} className="draft-dashboard" aria-label="草稿库" tabIndex={-1}>
    ...
    {drafts.length === 0 ? <p>还没有已保存的草稿。</p> : <ul className="draft-list">{drafts.map((draft) => <li key={draft.id}><DraftThumbnail draft={draft} /><div><h3>{draft.meta.title || '未命名文章'}</h3><p>{draft.meta.slug || '尚未设置 Slug'} · 修改于 {new Date(draft.updatedAt).toLocaleString('zh-CN')}</p></div><div className="draft-actions"><button type="button" disabled={disabled} onClick={() => void onOpen(draft)}>打开</button><button type="button" disabled={disabled} onClick={() => void draftRepository.duplicate(draft.id).then(refresh, (cause: unknown) => setError(errorMessage(cause)))}>复制</button><button ref={renameTrigger} type="button" disabled={disabled} onClick={(event) => { renameTrigger.current = event.currentTarget; setRenameDraft(draft); setRenameValue(draft.meta.title) }}>重命名</button><button type="button" disabled={disabled} onClick={() => void downloadDraft(draft)}>导出草稿</button><button ref={deleteTrigger} type="button" disabled={disabled} onClick={(event) => { deleteTrigger.current = event.currentTarget; setPendingDelete(draft) }}>删除</button></div></li>)}</ul>}
    {pendingDelete ? <AccessibleDialog title="删除草稿？" onClose={closeDelete} returnFocus={() => deleteTrigger.current ?? dashboardRef.current}><p>“{pendingDelete.meta.title || '未命名文章'}”及其图片将从本浏览器移除。</p><div className="dialog-actions"><DialogClose>{(close) => <button type="button" onClick={close}>取消</button>}</DialogClose><button type="button" onClick={() => void draftRepository.delete(pendingDelete.id).then(() => { closeDelete(); dashboardRef.current?.focus(); return refresh() }, (cause: unknown) => setError(errorMessage(cause)))}>删除草稿</button></div></AccessibleDialog> : null}
