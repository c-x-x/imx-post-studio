...
export function buildPreviewDocument({ meta, rendered, css, theme }: PreviewDocumentInput): string {
  ...
  return `<style>${shadowCss(css)}</style><style>${shadowCss(previewAccessibilityCss)}</style><style>${shadowCss(studioPreviewBehaviorCss)}</style><div class="preview-html" data-theme="${theme}"><div class="preview-body is-article-page">${symbols}<article class="article-page${toc ? '' : ' article-page-no-toc'}"><header class="article-header"><h1 class="article-title">${escapeHtml(meta.title)}</h1><div class="article-meta"><span><svg width="18" height="18"><use href="#icon-calendar"></use></svg> 发布于 ${escapeHtml(formatDate(meta.date))}</span>${categories ? `<span><svg width="18" height="18"><use href="#icon-folder"></use></svg>${categories}</span>` : ''}<span><svg width="18" height="18"><use href="#icon-clock"></use></svg> 阅读时长 ${rendered.readingMinutes} 分钟</span><span>${rendered.wordCount} 字</span></div>${tags ? `<div class="post-card-tags article-tags">${tags}</div>` : ''}</header><div class="layout-with-sidebar"><div class="main-content"><div class="article-content">${rendered.html}</div></div>${toc}</div></article></div></div>`
}
