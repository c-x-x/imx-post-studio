...
作品页支持删除远端文章：先展示警告确认框，明确目标仓库、分支和目录；确认后以一个提交
删除该文章目录（含封面、图片及附件），博客重新部署后下线。本地草稿和待提交修改保留，
远端版本变化则停止删除；误删可通过 GitHub 提交历史恢复。
...
浏览器部分使用 `playwright.ci.config.ts` 按风险分配 17 个用例，使用 2 个 worker 并行：
Chromium 运行全部 10 条关键流程；Firefox 只重复表格和粘贴图片 2 条；WebKit 重复
表格、粘贴图片、移动端目录、文章导入导出与 WebP 封面处理 5 条。
写作/草稿/模拟 GitHub 推送、ZIP 安全和代码块等通用流程不再在三种浏览器重复执行。
仅修改 `README.md` 或 `docs/` 下 Markdown 文档时，跳过浏览器安装和检查，其他检查照常运行。
无法确定变更范围或手动触发时不会跳过浏览器检查。
失败时保留 7 天 HTML 报告、截图及重试 trace，不全程录制视频。
需要全面检查时，在 Actions → CI → Run workflow 勾选 `full_browser_suite`。
字体和多尺寸 Dock 布局检查归入手动全面检查，不再夹在安全、推送等关键流程中。
