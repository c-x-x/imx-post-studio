...
文章发生修改后默认等待约 800ms 自动保存，可在设置中选择 0.4、0.8 或 1.5 秒。点击首页或 Studio 标志只切换页面，不会
...
1. 从首页点击“写作”进入编辑器，在左侧“属性”填写标题、Slug、日期、分类、标签、
   摘要和封面；“大纲”用于定位章节。
2. 桌面端“写作”默认使用“即时排版”模式；它基于 TipTap/ProseMirror，直接编辑标题、
   强调、引用、列表、任务、表格、代码块和本地图片等结构化内容。右侧“排版”按文字样式、
   段落结构、插入内容和编辑模式分组，正文图片也在这里管理。点击“源代码”
...
3. 编辑内容会自动保存，不再提供手动保存按钮。“草稿”分为“本地草稿”和“待提交作品”，
   ...
   多个窗口同时编辑时会阻止旧版本覆盖较新草稿；删除后的草稿也不会被旧窗口自动保存恢复。
   遇到冲突时请先导出当前修改备份，再重新打开较新草稿。
4. 点击右侧“预览文章”打开整屏文章预览；预览沿用 Studio 当前的浅色或深色主题。
5. 右侧“文档”集中新建、推送和导入导出；“备份草稿”用于可携带备份；“导出文章”会检查媒体引用，并可选择保留
   `draft = true` 或改为 `draft = false`。

手机端仅支持 CodeMirror 源代码编辑，设置中的“即时排版”不可选择，但不会覆盖桌面端保存的
编辑模式偏好。写作页不再显示“设置 / 写作 / 工具”切换栏，编辑区顶部提示栏内只提供
“属性”和“文档”两个工具入口，分别弹出文章属性（含封面）和文档操作面板。
关闭面板不会清空正文；从文档面板插入图片会返回写作，并在原光标位置插入 Markdown。

## Studio 设置

导航右侧的设置按钮提供五类本机偏好：通用、编辑器、图片、发布、安全与数据。当前可配置
自动保存等待时间，新文章是否默认显示目录或设为精选，编辑器默认使用即时排版或源代码，
编辑器字体（阅读、系统无衬线、等宽、霞鹜文楷或得意黑），封面最大尺寸与 WebP 质量，以及 GitHub Commit 信息模板。艺术字体采用
OFL 授权并随 Studio 自托管，仅在选择对应字体时加载。Commit 模板支持 `{title}` 和
`{slug}`；服务端仍会独立验证提交信息和文章完整性。恢复默认设置只删除这些偏好，不会
删除 IndexedDB 中的草稿、文章图片、GitHub 作品或登录状态。设置数据只保存在当前浏览器，
历史草稿不会因修改默认值而被批量改写。

手输 `#` 到 `######` 加空格的标题，以及 `**加粗**`、`*斜体*`、`~~删除线~~`、
行内代码等语法时，当前段落保留淡色标记；回车或将光标移到其他段落后再转为排版效果。
自动保存与源代码模式保留这些原始标记，不会把星号误存为转义文本。右侧格式按钮仍立即
生效。编辑区顶部持续显示保存状态，并按选区、表格、代码块、待排版段落和源码模式显示
操作提示；提示不代替保存成功或失败信息。
...
- 单个源图片上限为 25 MiB、4000 万像素，单边不超过 16384 像素；导入前检查格式、
  尺寸并实际解码，拒绝损坏图片。正文图片单次添加不超过 50 张、合计 100 MiB。
...
- 预览目录包含 H1–H6，正文标题锚点和脚注链接在预览内部跳转；保留安全的相对链接及电话链接。
...
- 自动保存失败时，页面会提供“紧急导出恢复备份”。该 ZIP 使用 Studio 专用的
  ...
  只能通过“导入紧急恢复 ZIP”恢复。
- 已有独立的 `index.md` 和图片时，可展开“从 index.md 和图片导入”。导出的 front matter
  包含 `slug`，因此无封面文章也可独立重新导入，不必依赖 ZIP 目录推断。
...
勾选“精选文章”会在 Front Matter 写入 `featured = true`；未勾选时为 `false`。博客首页优先展示精选文章，最多展示数量由博客主题控制。
...
浏览器部分使用 `playwright.ci.config.ts` 按风险分配 18 个用例，使用 2 个 worker 并行：
Chromium 运行全部 11 条关键流程（含跨窗口草稿冲突保护）；Firefox 只重复表格和粘贴图片 2 条；WebKit 重复
