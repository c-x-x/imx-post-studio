...
test('renames stay synchronized and stale tabs cannot overwrite or resurrect drafts', { tag: '@critical' }, async ({ page, context }) => {
  await page.goto('/')
  await navigate(page, '写作')
  const editor = page.getByRole('textbox', { name: 'Markdown 编辑器' })
  await page.getByLabel('标题').fill('原始名称')
  await editor.fill('原始正文')
  await expect(page.getByRole('status')).toContainText('已保存到本地草稿')
  await navigate(page, '草稿')
  await page.getByRole('button', { name: '重命名', exact: true }).click()
  await page.getByRole('dialog', { name: '重命名草稿' }).getByLabel('标题').fill('更新名称')
  await page.getByRole('button', { name: '保存名称' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await navigate(page, '写作')
  await expect(page.getByLabel('标题')).toHaveValue('更新名称')
  await editor.fill('重命名后正文')
  await expect(page.getByRole('status')).toContainText('已保存到本地草稿')

  const other = await context.newPage()
  await other.goto('/')
  await navigate(other, '草稿')
  await expect(other.getByRole('heading', { name: '更新名称' })).toBeVisible()
  await other.getByRole('button', { name: '打开', exact: true }).click()
  await other.getByRole('textbox', { name: 'Markdown 编辑器' }).fill('另一个窗口的较新正文')
