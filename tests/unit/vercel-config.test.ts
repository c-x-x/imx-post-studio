...
describe('Vercel static delivery configuration', () => {
  ...
  it('applies the required global browser security policy', async () => {
    ...
    expect(csp).toBe("default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' blob: data:; font-src 'self'; connect-src 'self'; worker-src 'self' blob:; frame-src 'self' blob:; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'")
    const scriptSources = csp?.split(';').map((directive) => directive.trim()).find((directive) => directive.startsWith('script-src '))?.split(/\s+/).slice(1)
    const workerSources = csp?.split(';').map((directive) => directive.trim()).find((directive) => directive.startsWith('worker-src '))?.split(/\s+/).slice(1)
