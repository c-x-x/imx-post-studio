...
新文章从“写作”开始，自动进入本地草稿。自动保存仍只写 IndexedDB；只有确认推送才更新主分支。
...
5. 成功后实际删除该本地草稿及图片、清空编辑区，作品页读取主分支的新版本。
   文章的 draft 标记不会自动改为 false；是否公开发布由博客构建决定。
...
未关联文章可作为新文章推送，但不能覆盖同名文章。关联文章不能直接改 Slug；目录迁移、
删除远端整篇文章暂不提供。导入替换关联草稿时取消关联，避免意外覆盖另一篇远端文章。
